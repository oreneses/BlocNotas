...
<div class="container">
    <div class="row">
        <div class="col-12 col-sm-6 col-md-6 col-lg-6 mx-auto">
            <form class="bg-white shadow rounded p-3" method="POST" action="{{ route('notes.update', $note) }}">
                @csrf @method('PATCH')
                <h1 class="display-6 mb-4">Editar nota:</h1>
                ...
                <div class="form-group">
                    <label class="px-2 py-2" for="text"> Descripción de la nota </label><br>
                        <textarea class="form-control bg-light shadow-sm @error('text') is-invalid @else border-1 @enderror"
                            ...
                            name="text"
                            value=""
                            placeholder="Escribe aquí la descripción de la nota">{{$note->text}}</textarea><br>
                            {!! $errors->first('text', '<small style="color:red">:message</small><br>') !!}
                <div>
                <br>
                <div class="form-group" style="margin-top: 15px">
                    <button class="btn btn-lg btn-block py-1" style="background-color: #38B2AC; color:white"> Actualizar </button>
